...
module.exports = class BaseModel {
  constructor(name, schema) {
    ...
    this.schema = Joi.object().keys(Object.assign({}, schema, { id: Joi.number(), nom: Joi.string().required(), prenom: Joi.string().required() }));
  ...
  getById(studentId) {
    if (typeof studentId === 'string') studentId = parseInt(studentId, 10);
    const item = this.items.find(i => i.studentId === studentId); // changer "id" en "studentId"
    if (!item) throw new NotFoundError(`Cannot get ${this.name} studentId=${studentId} : not found`);
    return item;
  }
